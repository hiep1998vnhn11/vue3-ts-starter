  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      function getRelativeTime(timeStamp: number) {
        ...
        let resStr = ''
        let dirStr = isBefore ? 'component.time.before' : 'component.time.after'

        if (diff < ONE_SECONDS) {
          resStr = 'component.time.just'
          // Less than or equal to 59 seconds
        } else if (diff < ONE_MINUTES) {
          resStr = parseInt(diff / ONE_SECONDS) + 'component.time.seconds' + dirStr
          // More than 59 seconds, less than or equal to 59 minutes and 59 seconds
        } else if (diff >= ONE_MINUTES && diff < ONE_HOUR) {
          resStr = Math.floor(diff / ONE_MINUTES) + 'component.time.minutes' + dirStr
          ...
          resStr = Math.floor(diff / ONE_HOUR) + 'component.time.hours' + dirStr
          ...
          resStr = Math.floor(diff / ONE_DAY) + 'component.time.days' + dirStr
